...
import 'package:multiple_image_camera/imagePreview.dart';

class MultipleImageCamera extends StatefulWidget {
  @override
  State<MultipleImageCamera> createState() => _MultipleImageCameraState();
}

class _MultipleImageCameraState extends State<MultipleImageCamera>
  ...
  List<MediaModel> _imageList = <MediaModel>[];
  late int _currIndex;
  ...
  addImages(XFile image) {
    setState(() {
      ...
      _animation = Tween<double>(begin: 400, end: 1).animate(scaleAnimation =
          CurvedAnimation(
  ...
  takePicture() async {
    ...
    try {
      ...
      setState(() {
        addImages(image);
        player.load('camerashutterios.mp3');
              ...
              ? GestureDetector(
                  onTap: () {
                    for (int i = 0; i < imageFiles.length; i++) {
                      ...
                      _imageList.add(new MediaModel.blob(
